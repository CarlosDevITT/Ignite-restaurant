...
// Contrato de "jogo" que qualquer módulo futuro em games/ deve seguir:
//   createGame({ canvas, onScoreChange, onStateChange }) => {
//     start, pause, resume, restart, setDirection, destroy, getScore, getState
//   }
// Não depende de Supabase, de pedidos, de DOM fora do <canvas> recebido, nem de
// nenhum outro jogo — pode ser copiado para tetris.js/pong.js/etc. sem tocar aqui.
...
const BASE_INTERVAL_MS = 165;
const MIN_INTERVAL_MS = 72;
const INTERVAL_STEP_MS = 5;
...
export function createSnakeGame({
  ...
  const step = () => {
    ...
    // Fix: ao não comer, a cauda (último segmento) sai da posição dela neste
    // mesmo passo — mover a cabeça para lá NÃO é colisão. Só conta como
    // colisão real se a próxima célula ainda estará ocupada depois do passo.
    const willEat = Boolean(food) && next.x === food.x && next.y === food.y;
    const bodyAfterMove = willEat ? snake : snake.slice(0, -1);
    if (bodyAfterMove.some((segment) => segment.x === next.x && segment.y === next.y)) { gameOver(); return; }
