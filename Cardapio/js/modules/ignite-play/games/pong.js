...
export function createPongGame({ canvas, onScoreChange, onStateChange } = {}) {
  ...
  const paddle = { x: 18, y: 110, w: 8, h: 62, speed: 5 };
  const ai = { x: 294, y: 110, w: 8, h: 62 };
  ...
  const reset = () => { paddle.y = 109; ai.y = 109; input = 0; emitScore(0); resetBall(1); draw(); };
  const draw = () => {
  ...
  const frame = () => {
    ...
    paddle.y = Math.max(0, Math.min(280-paddle.h, paddle.y + input*paddle.speed));
    ...
    if (ball.vx < 0 && hit(paddle)) { ball.x = paddle.x+paddle.w+ball.r; ball.vx = Math.abs(ball.vx)*1.035; ball.vy += input*.35; emitScore(score+10); }
  ...
  const setDirection = (dir) => { input = dir==='up' ? -1 : dir==='down' ? 1 : 0; };
