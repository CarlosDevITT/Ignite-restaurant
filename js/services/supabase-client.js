...
// Every Cardápio browser gets a persisted Supabase Auth identity. This is what
// allows RLS to isolate orders by orders.user_id without asking the customer
// for email/password. Existing signed-in sessions are preserved.
export async function ensureCustomerSession() {
  ...
  customerSessionPromise = (async () => {
    ...
    if (error) {
      console.error('[Auth] Não foi possível criar sessão anônima do cliente.', {
        code: error.code,
