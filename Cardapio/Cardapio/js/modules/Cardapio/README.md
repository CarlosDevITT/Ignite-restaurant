...
- `js/modules/ignite-play`: mini game Snake (visual Game Boy) exibido após o pedido ser criado, enquanto o cliente aguarda o preparo. Modal 100% criado via JS (`IgnitePlay.show/hide/destroy/isOpen/getScore`), sem dependência de Supabase — o fechamento automático é decidido em `js/app.js`, que observa o status do pedido pelo `orders.watchOrder(...)` já existente em `js/modules/orders.js`.
