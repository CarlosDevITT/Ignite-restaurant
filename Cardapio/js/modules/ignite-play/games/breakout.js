...
export function createBreakoutGame({ canvas, onScoreChange, onStateChange } = {}) {
  ...
  let state = BREAKOUT_STATE.READY, raf = null, destroyed = false, score = 0, input = 0;
  const paddle = { x: 124, y: 255, w: 72, h: 8, speed: 6 };
  ...
  const reset = () => { paddle.x=124; ball.x=160; ball.y=220; ball.vx=3.1; ball.vy=-3.1; input=0; emitScore(0); buildBricks(); draw(); };
  const draw = () => {
  ...
  const frame = () => {
    ...
    paddle.x = Math.max(0, Math.min(320-paddle.w, paddle.x + input*paddle.speed));
  ...
  const setDirection=(dir)=>{ input=dir==='left'?-1:dir==='right'?1:0; };
